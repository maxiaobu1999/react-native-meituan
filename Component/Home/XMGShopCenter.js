...
// 引入外部的json数据
var Home_D5 = require('../../LocalData/like.json');
...
var ShopCenter = React.createClass({
    getDefaultProps(){
        ...
        return{
            popToHomeView: null,
            api_url:'http://maqinglong-1253423006.costj.myqcloud.com/rnshop/like2.json'
    ...
    renderAllItem(){
        ...
        var itemArr = [];
        // 取出数据
        var shopData= this.state.dataSource===null?0:this.state.dataSource;
        // 遍历
        for (var i=0; i<shopData.length; i++){
           ...
           // 创建组件装入数组
            itemArr.push(
                <ShopCenterItem
                    shopImage = {data.deal.deal_img}
                    shopSale = {data.deal.deal_name}
                    shopName = {data.deal.deal_range}
                    detailurl = {data.deal.deal_url}
                    key={i}
